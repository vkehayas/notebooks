...
knitr::opts_chunk$set(fig.path = "img/glmm-theory/",
                      cache = TRUE,
                      autodep = TRUE,
                      dpi = 300,
                      fig.asp = 0.5)
...
# Initialize
library(ggplot2) # Development version 2.2.1.9000
library(cowplot, warn.conflicts = FALSE, quietly = TRUE)
set.seed(0) # For reproducible results
```
```{r x, echo=FALSE}
x = sort(rep(c(-1, 1), 100))
```
```{r data_distributional_assumption, echo=FALSE}
y = x * rnorm(100, 1, 0.5)
df = data.frame(x, y)
```
```{r outcome_distribution, echo=FALSE}
my_theme = 
  theme_classic() +
...
edp1 = ggplot(df, aes(x, y)) + 
  geom_point(size = 2, alpha = 0.2) + 
  geom_smooth(aes(x = as.numeric(x)),
              formula = y ~ x,
              method = "lm",
              colour = "black") +
  ylim(yAxisLim) +
  scale_x_continuous(limits = xAxisLim, breaks = c(-1, 1)) +
  my_theme
edp2 = ggplot(df) + 
  geom_histogram(aes(y), bins = 40) +
  coord_flip(xlim = yAxisLim) +
...
mdl = lm(y ~ x)
```
```{r simple_linear_model_residuals, echo=FALSE}
...
qqMdl = PlotQQ(mdl, my_theme)
qqY = ggplot(df, aes_string(sample = "y"), alpha = 0.3) +
  stat_qq_line(size = 1,
               linetype = "dashed",
               alpha = 0.5) +
  stat_qq(alpha = 0.3) +
